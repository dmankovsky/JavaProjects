...
public class NoteService {
    ...
    public int addNote(SingleNote note, int userId){
        return noteMapper.addNote(new SingleNote(0, note.getNoteTitle(),
                note.getNoteDescription(), userId));
    }

    public int editNote(SingleNote note){
    ...
    public boolean isOnlyNote(Integer userId, String noteTitle, String noteDescription){
        Optional<SingleNote>note =
                Optional.ofNullable(noteMapper.isOnlyNote(userId, noteTitle, noteDescription));
        return (note.isEmpty());
    }

    public List<SingleNote> getNotesFromUserId(Integer userId){
